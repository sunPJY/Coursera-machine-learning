...
%Part 1
%map vector y into a binary vector, y.shape 5000*10
y_map= zeros(m, num_labels);
for i = 1:m
    y_map( i, y(i) ) = 1;
end
...
J = (-1/m) * sum( ( sum(  y_map .* log(H) + (1-y_map) .* log(1-H)  ) ) );           %cost function without regularization
