...
lambda = 3;                 %using lambda from Part 8
[theta] = trainLinearReg(X_poly, y, lambda);
...
%         % get random sequence
%         rand_seq = randperm(m);
%         rand_seq = rand_seq(1:i);
%         % randomly choose training set & cross validation set
%         X_poly_rand = X_poly(rand_seq,:);
%         y_rand = y(rand_seq,:);
%         X_poly_val_rand = X_poly_val(rand_seq,:);
%         yval_rand = yval(rand_seq,:);
%         % train linear regression using the randomly chosen training set
%         [theta] = trainLinearReg(X_poly_rand, y_rand, lambda);
%         
%         error_train = linearRegCostFunction(X_poly_rand, y_rand, theta, 0);
%         error_val = linearRegCostFunction(X_poly_val_rand, yval_rand, theta, 0);
%         error_train_sum = error_train_sum + error_train;
%         error_val_sum = error_val_sum + error_val;
%     end
%     error_train(i) = error_train_sum/k;
%     error_val(i) = error_val_sum/k;
% end
% 
% plot(1:m, error_train, 1:m, error_val);
% title(sprintf('Polynomial Regression Learning Curve (lambda = %f)', lambda));
